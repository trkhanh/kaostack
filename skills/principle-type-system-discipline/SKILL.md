...
Applies to any typed language. Skills like [[_ai/_skill/kaostack/skills/typescript-best-practices/SKILL|typescript-best-practices]] ground it in specific syntax.
...
- **External data is untyped until parsed.** RPC payloads, JSON, IPC messages, CLI args, config files, environment variables, database rows. Have a parse function at every boundary that turns unstructured input into the typed model. See the [[_ai/_skill/kaostack/skills/principle-boundary-discipline/SKILL|boundary-discipline]] principle skill for where to put validation.
...
- **Derive types from authoritative schemas.** When a protocol buffer, OpenAPI spec, GraphQL schema, database migration, or design-system token file defines a shape, derive from it instead of hand-rolling a parallel type. See the [[_ai/_skill/kaostack/skills/principle-encode-lesson-in-structure/SKILL|encode-lessons-in-structure]] principle skill.
