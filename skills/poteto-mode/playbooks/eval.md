...
4. **Spawn N parallel candidates** on different models per the [[_ai/_skill/kaostack/skills/arena/SKILL|arena]] skill's Phase B. Each works in its own sanitized dir. Same prompt to each.
5. **Spawn one blinded judge** on a different model family per the [[_ai/_skill/kaostack/skills/arena/SKILL|arena]] skill's Phase C. Judge sees outputs by sanitized label and the rubric, never a model name.
